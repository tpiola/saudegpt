...
export const scriptAntiFlash = `(function(){try{var t=localStorage.getItem('${CHAVE}');var d=t? t==='dark' : window.matchMedia('(prefers-color-scheme: dark)').matches; if(d){document.documentElement.classList.add('dark');}}catch(e){}})();`;

export function ThemeProvider({ children }: { children: React.ReactNode }) {
  const [tema, setTema] = useState<Tema>("light");

  useEffect(() => {
    const inicial: Tema = document.documentElement.classList.contains("dark")
      ? "dark"
      : "light";
