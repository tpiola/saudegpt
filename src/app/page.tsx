...
/* ─── Trilha icon map ─── */
const TRILHA_ICON: Record<string, string> = {
  perfumaria: "sparkles",
  medicamentos: "pill",
  operacional: "shield",
  encantamento: "heart",
...
export default function HomePage() {
  ...
  return (
    <div className="relative min-h-screen bg-background text-foreground">
      {/* ═══ HERO — CINEMATIC ═══ */}
      <section className="relative flex min-h-[65vh] items-center overflow-hidden bg-[#020617]">
        <HeroVideo />
        ...
        <div className="relative z-40 mx-auto w-full max-w-7xl px-6 sm:px-8 lg:px-12">
          <div className="max-w-2xl">
            <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-green-400/20 bg-green-500/10 px-4 py-1.5 text-xs font-medium tracking-wide text-green-300 backdrop-blur-md shadow-[0_0_24px_rgba(76,161,93,0.12)]">
              ...
              <span className="ml-1">Formação completa para Atendentes</span>
            </div>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <a href="#feed" className="group inline-flex items-center gap-2 rounded-2xl bg-green-500 px-7 py-3.5 text-sm font-semibold text-white shadow-[0_8px_40px_rgba(76,161,93,0.35)] transition-all duration-300 hover:bg-green-600 hover:scale-[1.03] hover:shadow-[0_12px_50px_rgba(76,161,93,0.5)] active:scale-[0.97]">
                <Icon name="play" size={16} />
                Explorar conteúdo
              </a>
              <a href="#ser-aluno" className="group inline-flex items-center gap-2 rounded-2xl border-2 border-white/20 bg-white/5 px-7 py-3.5 text-sm font-semibold text-white backdrop-blur-sm transition-all duration-300 hover:border-white/40 hover:bg-white/10 active:scale-[0.97]">
      ...
      <div id="feed" className="mx-auto max-w-6xl px-4 py-12 sm:px-6 lg:px-8">
        ...
        <FadeUp>
          <div className="mb-10">
            <div className="mb-3 flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.15em] text-muted">
              <Icon name="book" size={14} className="text-green-500" />
            ...
            <div className="flex gap-3 overflow-x-auto pb-2 snap-x snap-mandatory scrollbar-none">
              {trilhas.slice(0, 6).map((t) => (
                <Link key={t.id} href={`/trilhas/${t.id}`}
                  ...
                  <span className="flex h-14 w-14 items-center justify-center rounded-full bg-gradient-to-br from-green-500/20 to-green-400/10 text-2xl ring-2 ring-green-400/20 group-hover:ring-green-400/40 transition-all">
                    {TRILHA_EMOJI[t.id] ?? "📚"}
                  </span>
        ...
        <div className="space-y-8">
          ...
          {/* POST: Simulação de Balcão */}
          <FadeUp>
            <div className="rounded-3xl border border-border/60 bg-surface overflow-hidden shadow-sm hover:shadow-md transition-shadow group">
              <div className="relative h-48 sm:h-56 overflow-hidden bg-forest-900">
                ...
                <div className="absolute bottom-4 left-5 flex items-center gap-2">
                  <span className="flex h-6 w-6 items-center justify-center rounded-full bg-green-500/30 backdrop-blur-sm">
                    <Icon name="check" size={12} className="text-green-300" />
                  </span>
              ...
              <div className="p-5 sm:p-6">
                <div className="flex items-center gap-2 mb-2 text-xs text-muted">
                  <Icon name="play" size={12} className="text-green-500" />
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-2">
                  <div className="rounded-xl border border-emerald-200/50 bg-emerald-50/50 dark:bg-emerald-950/20 p-4 hover:border-emerald-400/50 transition-colors">
                  ...
                  <div className="rounded-xl border border-rose-200/50 bg-rose-50/50 dark:bg-rose-950/20 p-4 hover:border-rose-400/50 transition-colors">
          ...
          {/* POST: Grade Curricular */}
          <FadeUp delay={100}>
            <div className="rounded-3xl border border-border/60 bg-surface overflow-hidden shadow-sm hover:shadow-md transition-shadow group">
              <div className="relative h-48 sm:h-56 overflow-hidden bg-forest-900">
                <Image src={IMG.medicamentos} alt="" fill className="object-cover opacity-60 group-hover:scale-105 transition-transform duration-700" sizes="100vw" />
                <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />
                <div className="absolute bottom-4 left-5 flex items-center gap-2">
                  <Icon name="book" size={14} className="text-orange-300" />
              ...
              <div className="p-5 sm:p-6">
                ...
                <div className="mt-4 space-y-2">
                  {trilhas.map((t: Trilha, i: number) => (
                    <Link key={t.id} href={`/trilhas/${t.id}`}
                      className="flex items-center gap-3 rounded-xl p-3 transition-all duration-200 hover:bg-muted/30 group/item">
                      ...
                      <Icon name="arrow" size={14} className="text-muted group-hover/item:translate-x-1 transition-transform" />
                    </Link>
                  ))}
                </div>
              </div>
            ...
            <div className="rounded-3xl border border-border/60 bg-surface overflow-hidden shadow-sm hover:shadow-md transition-shadow">
              <div className="relative h-40 sm:h-48 overflow-hidden bg-gradient-to-br from-forest-600 via-forest-500 to-forest-700">
                ...
                <div className="absolute bottom-4 left-5 right-5 flex items-end justify-between">
                  <div>
                    <span className="rounded-full bg-green-500/20 border border-green-400/30 px-3 py-1 text-[11px] font-semibold text-green-300 flex items-center gap-1.5">
                  ...
                  <Link href="/ranking" className="flex items-center gap-1 text-xs text-white/70 hover:text-white transition-colors">
                    Ver ranking
              ...
              <div className="p-5 sm:p-6">
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <Link href="/ranking" className="flex flex-col items-center gap-2 rounded-xl bg-muted/30 p-4 transition-all hover:bg-muted/50 hover:-translate-y-0.5">
                    <Icon name="trending" size={20} className="text-green-500" />
          ...
          {/* POST: Conteúdos gratuitos */}
          <FadeUp delay={300}>
            <div className="rounded-3xl border border-border/60 bg-surface overflow-hidden shadow-sm hover:shadow-md transition-shadow">
              <div className="p-5 sm:p-6">
                <div className="flex items-center justify-between mb-4">
                  <h3 className={h3}>Conteúdos gratuitos</h3>
                  <Link href="/curiosidades" className="flex items-center gap-1 text-xs font-medium text-green-600 dark:text-green-400 hover:underline">
                    Ver todos
                    <Icon name="arrow" size={12} />
                  </Link>
                </div>
            ...
            <FadeUp>
              <div>
                ...
                <div className="mt-6 space-y-3">
                  {[
                    { icon: "book", text: "Acesso completo — 6 trilhas, 142+ aulas" },
                    { icon: "star", text: "Gamificação — XP, ranking, badges, missões" },
                    { icon: "check", text: "Certificado ao finalizar a formação" },
                  ].map((b) => (
                    <div key={b.text} className="flex items-center gap-3">
                      <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-white/5 ring-1 ring-white/10">
                        <Icon name={b.icon as any} size={16} className="text-green-400" />
                      </span>
