...
export function ModoSobrevivencia({ titulo, questoes }: ModoSobrevivenciaProps) {
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-1">
              {Array.from({ length: 3 }, (_, i) => (
                <motion.span
                  key={i}
                  animate={i >= vidas ? { scale: 0, opacity: 0 } : { scale: 1, opacity: 1 }}
                  transition={{ type: "spring", stiffness: 300, damping: 12 }}
                  className={i < vidas ? "text-red-500" : "text-surface-3"}
                >
                ...
                <div className="flex items-start gap-3">
                  <span
                    className={`mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-base ${
                      acertou ? "bg-emerald-100 dark:bg-emerald-900/40" : "bg-red-100 dark:bg-red-900/40"
                    }`}
                  >
                    {acertou ? "✅" : "💡"}
                  </span>
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-semibold text-foreground">
                      {acertou ? "Sobreviveu!" : `Errou! ${vidas > 0 ? `Restam ${vidas} vidas` : "Game Over!"}`}
                    </p>
