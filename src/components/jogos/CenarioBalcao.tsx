    ...
    return (
      <Card>
        <div className="flex flex-col items-center gap-3 text-center">
          ...
          <div>
            <h3 className="text-lg font-bold">{titulo} — resultados</h3>
            <p className="mt-1 text-2xl font-extrabold">
              {acertos}/{questoes.length}
              <span className={`ml-2 text-sm ${pct >= 70 ? "text-cyan-500" : "text-orange-500"}`}>
                ({pct}%)
              </span>
            </p>
    ...
    <Card>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-cyan-100 dark:bg-cyan-900/40">
              <Icon name="users" size={14} className="text-cyan-600" />
            </span>
            <span className="text-sm font-medium text-subtle">{titulo}</span>
          </div>
          <span className="text-xs font-bold text-cyan-600">{pontos} pts</span>
        </div>
      ...
      >
        ...
        <div className="mt-4 rounded-xl border border-cyan-200/40 bg-gradient-to-br from-cyan-50/60 to-sky-50/60 p-4 dark:border-cyan-700/30 dark:from-cyan-900/10 dark:to-sky-900/10">
          <div className="flex items-start gap-3">
            <span className="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-cyan-100 dark:bg-cyan-800/50">
              <Icon name="message-circle" size={18} className="text-cyan-600" />
            </span>
            <div>
        ...
        <div className="mt-4 space-y-3">
          {q.opcoes.map((op, i) => {
            let estilo = "border-border hover:border-cyan-300 hover:bg-cyan-50/20 dark:hover:bg-cyan-900/10";
            if (escolha != null) {
              if (i === q.correta)
                estilo = "border-cyan-400 bg-cyan-50/60 dark:bg-cyan-900/20 ring-1 ring-cyan-400/30";
              else if (i === escolha)
                ...
                <span
                  className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full border text-[11px] font-bold ${
                    escolha != null && i === q.correta
                      ? "border-transparent bg-gradient-to-r from-cyan-500 to-sky-600 text-white shadow-sm"
            ...
            >
              <div
                className={`rounded-xl border px-4 py-4 ${
                  acertou
                    ? "border-cyan-400/20 bg-gradient-to-br from-cyan-500/10 to-cyan-400/5"
                ...
                <div className="flex items-start gap-3">
                  <span
                    className={`mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-base ${
                      acertou ? "bg-cyan-100 dark:bg-cyan-900/40" : "bg-orange-100 dark:bg-orange-900/40"
                    }`}
                  >
                    {acertou ? "✅" : "💡"}
                  </span>
